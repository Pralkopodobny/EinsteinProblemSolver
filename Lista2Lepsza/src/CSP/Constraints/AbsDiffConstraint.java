...
public class AbsDiffConstraint implements BinaryConstraint<Integer> {
    private int first, sec;
    private ArrayList<Integer> solution;
    private Integer value;

    public AbsDiffConstraint(int first, int sec, Integer value) {
        this.first = first;
        this.sec = sec;
        this.value = value;
    }
    ...
    public boolean satisfied() {
        ...
        return (Math.abs(solution.get(first) - solution.get(sec))) == value;
    }
    ...
    private boolean AC3DomainsStep(ArrayList<Integer> dom1, ArrayList<Integer> dom2){
        ...
        for(int i = 0; i < dom1.size(); i++){
            ...
            for (Integer integer : dom2) {
                if (Math.abs(integer - val) == value) {
                    remove = false;
                    break;
                }
            }
            if(remove) dom1rm.add(i);
        }
        if(dom1rm.size() == 0){
            return false;
        }
    ...
    public BinaryConstraint<Integer> reversed() {
        return new AbsDiffConstraint(sec, first, value);
    }
    ...
    public boolean equals(Object o) {
        ...
        AbsDiffConstraint that = (AbsDiffConstraint) o;
    ...
    public void simplifyDomains(Integer value, ArrayList<ArrayList<Integer>> domains) {
        ...
        for(int i = 0; i < domain.size(); i++){
            if(Math.abs(domain.get(i) - value) != this.value)
                dom2rm.add(i);
        }
